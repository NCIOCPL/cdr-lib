...
#
# $Id: cdrpubcgi.py,v 1.1 2002-06-13 19:18:24 bkline Exp $
...
""" % (self.jobId, self.jobId)
    ...
    def __resetParamsByJobId(self):
        ...
        while not rs.EOF:
            ...
            # Modify INSERT to do a good job.
    ...
    def __insertDoc(self, docId, version):
        ...
        sql = """INSERT INTO pub_proc_doc (pub_proc, doc_id, doc_version)
                 VALUES (%d, %s, %s) 
                 WHERE NOT EXISTS (SELECT doc_id FROM pub_proc_doc
                        WHERE doc_id = %s)""" % (self.__procId, 
                                            docId, version, docId)
        self.__execSQL(sql)
