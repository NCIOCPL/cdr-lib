...
import sys, os, string, re, xml.dom.minidom, UnicodeToLatex
...
class latexDoc:
    def __init__(self, latex="", msgs=None, status=0, latexPassCount=1):
        self.latex          = latex
        self.msgs           = msgs
        self.status         = status
...
def makeLatex (Xml, docFmt, fmtType='', params=None):
    ...
    elif 1: # (type (Xml) == type (xml.dom.minidom.Document)):
        topNode = Xml.documentElement
    elif (type (Xml) != type (xml.dom.minidom.Element)):
        raise XmlLatexException (
